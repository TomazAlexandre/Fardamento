...
function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    catch (err) { setErro(err.message || 'Email ou senha incorretos'); }
    setLoading(false);
  }
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
          <form onSubmit={handleSubmit} className="space-y-3">
            <div><label className="block text-xs text-gray-500 mb-1">E-mail</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="seu@email.com" required className={inp} /></div>
            <div><label className="block text-xs text-gray-500 mb-1">Senha</label>
              <input type="password" value={senha} onChange={e => setSenha(e.target.value)} placeholder="••••••••" required className={inp} /></div>
            {erro && <p className="text-xs text-red-600 bg-red-50 rounded-lg px-3 py-2">{erro}</p>}
            <button type="submit" disabled={loading}
              className="w-full py-2.5 bg-emerald-600 text-white text-sm font-semibold rounded-xl hover:bg-emerald-700 transition-colors disabled:opacity-50">
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        {loadingData ? (
          ...
          <>
            {tab === 'orcamento' && (
              <OrcamentoTab produtos={produtos} orcamentos={orcamentos}
          ...
          <div className="flex items-center gap-3 mb-4 p-3 bg-amber-50 border border-amber-100 rounded-xl">
            ...
            {form.usarImposto && (
              <div className="flex items-center gap-1 ml-auto">
                <input type="number" value={form.imposto} onChange={e => setForm({ ...form, imposto: e.target.value })}
                  className="w-16 border border-amber-200 rounded-lg px-2 py-1 text-sm text-center bg-white" />
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <Field label={itemBanco.tipo === 'material' ? 'Material' : 'Serviço'}>
                    <select value={itemBanco.refId} onChange={e => setItemBanco({ ...itemBanco, refId: e.target.value })} className={inp}>
                      ...
                      {listaBanco.map(x => (
                        <option key={x.id} value={x.id}>{x.nome} — {fmt(x.valor)}/{x.medida}{itemBanco.tipo === 'material' && x.rendimento ? ` (rend.${x.rendimento})` : ''}</option>
                      ))}
                    </select>
                  </Field>
      ...
      {subTab === 'materiais' && (
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setEditM({ nome: '', valor: '', medida: 'metro', rendimento: '' })} className={btnPrimary}>+ Novo</button>
          </div>
          {editM && (
            <Card title={editM.id ? 'Editar Material' : 'Novo Material'}>
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Rendimento"><input type="number" step="0.01" value={editM.rendimento} onChange={e => setEditM({ ...editM, rendimento: e.target.value })} className={inp} placeholder="ex: 3" /></Field>
              </div>
              <div className="flex gap-2 justify-end mt-3">
                <button onClick={() => setEditM(null)} className={btnSecondary}>Cancelar</button>
                <button onClick={hSM} disabled={salvando} className={`${btnPrimary} disabled:opacity-50`}>{salvando ? 'Salvando...' : 'Salvar'}</button>
              </div>
            </Card>
          )}
          ...
          {editS && (
            <Card title={editS.id ? 'Editar Serviço' : 'Novo Serviço'}>
              ...
              <div className="flex gap-2 justify-end mt-3">
                ...
                <button onClick={hSS} disabled={salvando} className={`${btnPrimary} disabled:opacity-50`}>{salvando ? 'Salvando...' : 'Salvar'}</button>
              </div>
            </Card>
          )}
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
...
// ─── MODAL DADOS DO CLIENTE (PROTOCOLO) ──────────────────────────────────────
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6">
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <Field label="Razão Social / Nome" className="col-span-2">
              <input value={dados.razaoSocial} onChange={e => set('razaoSocial', e.target.value)} className={inp} placeholder="Nome da empresa ou pessoa" />
            </Field>
            <Field label="CNPJ / CPF">
              <input value={dados.cnpj} onChange={e => set('cnpj', e.target.value)} className={inp} placeholder="00.000.000/0001-00" />
            </Field>
            <Field label="Telefone">
              <input value={dados.telefone} onChange={e => set('telefone', e.target.value)} className={inp} placeholder="(00) 00000-0000" />
            </Field>
            <Field label="Endereço" className="col-span-2">
              <input value={dados.endereco} onChange={e => set('endereco', e.target.value)} className={inp} placeholder="Rua, número, bairro" />
            </Field>
            <Field label="Cidade / UF">
              <input value={dados.cidade} onChange={e => set('cidade', e.target.value)} className={inp} placeholder="Teresina / PI" />
            </Field>
            <Field label="E-mail">
              <input value={dados.email} onChange={e => set('email', e.target.value)} className={inp} placeholder="contato@empresa.com" />
            </Field>
            <Field label="Responsável pelo recebimento">
              <input value={dados.responsavel} onChange={e => set('responsavel', e.target.value)} className={inp} placeholder="Nome do responsável" />
            </Field>
            <Field label="Cargo">
              <input value={dados.cargo} onChange={e => set('cargo', e.target.value)} className={inp} placeholder="Gerente, Diretor..." />
            </Field>
          </div>
        </div>
        ...
        <div className="flex gap-3 justify-end mt-5">
          ...
          <button onClick={() => onGerar(dados)} className="px-4 py-2 bg-purple-600 text-white text-sm font-medium rounded-lg hover:bg-purple-700 transition-colors">
            📄 Gerar Protocolo PDF
          </button>
        </div>
      </div>
...
function HistoricoTab({ orcamentos, produtos, onDelete, onUpdateOrcamento }) {
  ...
  const [modalProtocolo, setModalProtocolo] = useState(null); // orçamento selecionado
  ...
  async function adicionarMovimento(orcId) {
    ...
    try {
      ...
      const { data, error } = await supabase.from('caixa').insert({
        ...
        user_id: user.id,
        tipo: novoMov.tipo,
        descricao: novoMov.descricao,
        valor: parseFloat(novoMov.valor),
        data: novoMov.data,
      }).select().single();
      if (error) throw error;
  ...
  async function gerarProtocolo(orc, dadosEmpresa) {
    // Salva os dados no orçamento para próxima vez
    try { await onUpdateOrcamento({ ...orc, dadosEmpresa }); } catch (e) { /* não bloqueia */ }
    const { exportarProtocoloPDF } = await import('../lib/pdf');
    await exportarProtocoloPDF({ ...orc, dadosEmpresa });
    setModalProtocolo(null);
  }

  async function exportarOrcPDF(orc) {
    const { exportarOrcamentoPDF } = await import('../lib/pdf');
    await exportarOrcamentoPDF(orc, produtos);
  }
  ...
  return (
    <>
      ...
      {modalProtocolo && (
        <ModalProtocolo
          orc={modalProtocolo}
          onClose={() => setModalProtocolo(null)}
          onGerar={(dados) => gerarProtocolo(modalProtocolo, dados)}
        />
      )}
            ...
            <Card key={orc.id}>
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  {temDadosEmpresa && <span className="ml-2 text-xs bg-purple-50 text-purple-600 px-1.5 py-0.5 rounded-full">dados empresa ✓</span>}
                </div>
                <div className="flex items-center gap-1.5 flex-wrap justify-end">
                  ...
                  <button onClick={() => exportarOrcPDF(orc)} className="text-xs text-orange-500 hover:text-orange-700 px-1.5">📄 Orç.</button>
                  <button onClick={() => setModalProtocolo(orc)} className="text-xs text-purple-500 hover:text-purple-700 px-1.5">📦 Protocolo</button>
                  <button onClick={() => abrirCaixa(orc.id)}
                    className={`text-xs px-2 py-0.5 rounded-full border transition-colors ${caixaEstaAberto ? 'bg-emerald-600 text-white border-emerald-600' : 'border-emerald-300 text-emerald-700 hover:bg-emerald-50'}`}>
              ...
              {caixaEstaAberto && (
                <div className="mt-4 pt-4 border-t-2 border-emerald-100">
                  ...
                  {loadingCaixa ? <p className="text-xs text-gray-400 text-center py-3">Carregando...</p> : (
                    <>
                      {movs.length === 0 && (
                        <p className="text-xs text-gray-400 text-center py-3 border border-dashed border-gray-200 rounded-lg mb-3">Nenhuma movimentação ainda</p>
                      )}
                      {movs.length > 0 && (
                        <div className="space-y-1 mb-3">
                          {movs.map(m => (
                            <div key={m.id} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-xs ${m.tipo === 'entrada' ? 'bg-emerald-50' : 'bg-red-50'}`}>
                              ...
                              <span className={`font-semibold w-20 text-right ${m.tipo === 'entrada' ? 'text-emerald-700' : 'text-red-600'}`}>
                      ...
                      <div className="bg-gray-50 rounded-xl p-3">
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          <Field label="Tipo">
                            <select value={novoMov.tipo} onChange={e => setNovoMov({ ...novoMov, tipo: e.target.value })} className={inp}>
                              <option value="entrada">✅ Entrada</option>
                              <option value="saida">❌ Saída</option>
                            </select>
                          </Field>
                          <Field label="Descrição">
                            <input value={novoMov.descricao} onChange={e => setNovoMov({ ...novoMov, descricao: e.target.value })} placeholder="Ex: Pagamento parcial" className={inp} />
                          </Field>
                          <Field label="Valor (R$)">
                            <input type="number" step="0.01" min="0" value={novoMov.valor} onChange={e => setNovoMov({ ...novoMov, valor: e.target.value })} className={inp} />
                          </Field>
                          <Field label="Data">
                            <input type="date" value={novoMov.data} onChange={e => setNovoMov({ ...novoMov, data: e.target.value })} className={inp} />
                          </Field>
                        </div>
                        <div className="flex justify-end mt-2">
                          <button onClick={() => adicionarMovimento(orc.id)} disabled={salvandoMov}
                            className={`${btnPrimary} text-xs py-1.5 disabled:opacity-50`}>
                            {salvandoMov ? 'Salvando...' : '+ Adicionar'}
                          </button>
                        </div>
                      </div>
                    </>
