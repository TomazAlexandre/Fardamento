// lib/db.js — banco de dados via Supabase (nuvem)
// Cada usuário vê só seus próprios dados (Row Level Security)
...
// ─── SEED: popula dados iniciais para usuário novo ────────────────────────────
...
export async function saveMaterial(m) {
  const { id, ...rest } = m;
  if (id) {
    const { data, error } = await supabase.from('materiais').update({ ...rest, updated_at: new Date().toISOString() }).eq('id', id).select().single();
    if (error) throw error;
    return data;
  } else {
    const { data, error } = await supabase.from('materiais').insert({ ...rest, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }).select().single();
    if (error) throw error;
    return data;
  }
}
...
export async function saveServico(s) {
  const { id, ...rest } = s;
  if (id) {
    const { data, error } = await supabase.from('servicos').update({ ...rest, updated_at: new Date().toISOString() }).eq('id', id).select().single();
    if (error) throw error;
    return data;
  } else {
    const { data, error } = await supabase.from('servicos').insert({ ...rest, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }).select().single();
    if (error) throw error;
    return data;
  }
}
...
export async function saveProduto(p) {
  const { id, ...rest } = p;
  if (id) {
    const { data, error } = await supabase.from('produtos').update({ ...rest, updated_at: new Date().toISOString() }).eq('id', id).select().single();
    if (error) throw error;
    return { ...data, custos: data.custos || [] };
  } else {
    const { data, error } = await supabase.from('produtos').insert({ ...rest, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }).select().single();
    if (error) throw error;
    return { ...data, custos: data.custos || [] };
  }
}
...
export async function saveOrcamento(o) {
  const { id, ...rest } = o;
  if (id) {
    const { data, error } = await supabase.from('orcamentos').update({ ...rest, updated_at: new Date().toISOString() }).eq('id', id).select().single();
    if (error) throw error;
    return data;
  } else {
    const { data, error } = await supabase.from('orcamentos').insert({ ...rest, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }).select().single();
    if (error) throw error;
    return data;
  }
}
...
export async function saveColaborador(c) {
  const { id, ...rest } = c;
  if (id) {
    const { data, error } = await supabase.from('colaboradores').update({ ...rest, updated_at: new Date().toISOString() }).eq('id', id).select().single();
    if (error) throw error;
    return data;
  } else {
    const { data, error } = await supabase.from('colaboradores').insert({ ...rest, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }).select().single();
    if (error) throw error;
    return data;
  }
}
